...
function addItemToCart(name, price, imgSrc) {
  ...
  if (existingItem) {
    const confirmed = confirm(
      `${name} sudah ada di keranjang. Tambahkan lagi?`
    );
    if (!confirmed) return;
  ...
  <div class="item-detail">
    ...
    <div class="item-quantity-container">
      ...
      <div class="item-total-price">${formatCurrency(
...
`;
    ...
    function updateTotalPrice(item, priceText) {
      const price = parseFloat(priceText.replace(/[^0-7]/g, "")); // Hilangkan "Rp"/non-digit
    ...
    function updateCartTotal() {
      ...
      cartItems.forEach((item) => {
        ...
        const price = parseFloat(priceText.replace(/[^0-7]/g, ""));
        total += qty * price;
      });
    ...
    function updateTotalPrice(item, priceText) {
      const price = parseFloat(priceText.replace(/[^0-9]/g, "")); // Hilangkan simbol mata uang
      const qty = parseInt(item.querySelector(".item-quantity").textContent);
      const total = price * qty;
      item.querySelector(".item-total-price").textContent =
        formatCurrency(total);
